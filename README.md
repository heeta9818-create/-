...
npm run test:db  # 44개(+17개) — 진짜 Postgres/PostgREST에 돌려 본다 (아래 참고)
...
supabase/reinstall.min.sql ← 위와 같되 주석 없는 판 (채팅에 붙여넣을 때)
...
```bash
npm run test:db        # 44개 — SQL만
npm run db:postgrest   # PostgREST 내려받기 (한 번만)
POSTGREST_BIN=.cache/postgrest npm run test:db   # 61개 — 저장소 코드까지
```
...
> **브라우저 자동 번역은 꺼 둘 것.** Supabase 대시보드가 한국어로 번역돼
> 있으면 SQL 편집기 안의 글자까지 번역돼서 읽을 수가 없다 (Run 버튼이
> "도망쳐"로 보이면 켜져 있는 것이다). 주소창의 번역 아이콘 → 원본 표시.
...
주석까지 빼서 더 짧은 `supabase/reinstall.min.sql` 도 있다. 채팅이나 좁은
화면에 통째로 붙여넣을 때 쓴다. 만들어지는 표·정책·함수는 완전히 같고,
테스트가 두 파일의 결과를 대조한다.
...
Vercel은 저장소의 **기본 브랜치**를 프로덕션으로 배포한다. 앱이 든 브랜치가
기본이 아니면 엉뚱한 걸 배포한다. 배포는 "성공"으로 뜨고 주소만 404가
나오기 때문에 원인을 짐작하기 어렵다.

두 가지 방법이 있고 아무거나 하나면 된다.

**A. Vercel에서 바꾸기** — 이미 Vercel에 있다면 이쪽이 빠르다.
프로젝트 → **Settings → Git → Production Branch** 를 앱이 든 브랜치로
바꾸고 Save.

**B. GitHub에서 기본 브랜치 바꾸기** — GitHub → 저장소 →
**Settings → General → Default branch**. 이쪽을 바꾸면 GitHub에서 파일을
찾을 때도 편해지므로, 시간이 되면 이것도 해 두는 게 낫다.

설정을 바꾼 뒤에는 **새 커밋을 밀어야** 그 브랜치가 배포된다. 설정만 바꾼다고
이미 끝난 배포가 다시 도는 것은 아니다.

> GitHub 앱이 연결돼 있으면 어느 브랜치든 밀 때마다 **Preview 배포**가
> 자동으로 만들어진다. 프로덕션 설정을 바꾸기 전에도 그 주소로 앱을 열어
> 볼 수 있다.
